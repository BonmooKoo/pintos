...
pid_t process_execute (const char *file_name) 
{
	...
	printf("process_execute: parent sleep\n");
	sema_down(&cur->load_sema);//wait for child to wake me up
	printf("process_execute: child exec succecc!\n");	
	palloc_free_page(fn_copy);
	return tid;
}
...
start_process (void *file_name_)
{
	...
	//load program
	printf("start_process : try to load %s\n",argv[0]);
	...
	int i;
	for(i=argc-1;i>=0;i++){
		//0xbf ff ff fc argv[3] = "bar"
		//0xbf ff ff ed argv[0] = "/bin/ls"
		*esp -= strlen(argv[i])+1;//'\0'
	...
	while((int)(*esp)%4!=0)
		*esp-=1;
	printf("load fin\n");
	// esp = -xbfff ff ec //alinged
	*esp -= 4;
	**(uint32_t**)esp=NULL;//last argv = null
	// 0xbf ff ff e8 = NULL (=argv[4])

	//argv[i] 주소 esp 에 저장
	for(i=argc-1;i>=0;i--){
		//0xbf ff ff e4 = point to 0xbf ff ff fc (argv[3])
		//0xbf ff ff d8 = argv[0]
		*esp-=4;
	...
	*esp -= 4;
	**(uint32_t**)esp= NULL;
	// 0x bf ff ff c8 = return add 
	//BM : tell parent i am ready
	
	printf("start_process : i am ready!\n");
	sema_up(&cur->parent->load_sema);
	palloc_free_page(file_name);
	timer_sleep(1000);
	//BM : end  
...
process_wait (tid_t child_tid) 
{
	...
	if(child==NULL||child->waited){ //it is terminated (NULL) or if it already waited child
		return -1;
	}
	child->waited=true;
	sema_down(&child->wait_sema);//tell child i am waiting
	int status = child->exit_status;
	list_remove(&child->child_elem); // child killed
	return status;
}
